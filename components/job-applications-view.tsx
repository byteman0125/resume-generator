...
import { defaultResumeData, APPLICATION_RESUME_STYLE, type ResumeData, type StoredProfileData } from "@/lib/resume-store";
...
import { Check, Copy, Download, ExternalLink, FileText, Loader2, ChevronUp, ChevronDown } from "lucide-react";
...
export function JobApplicationsView() {
  ...
  const [applicationModalFormatId, setApplicationModalFormatId] = useState<FormatId>(() => {
    ...
    try {
      ...
      if (stored && FORMAT_LIST.some((f) => f.formatId === stored)) return stored as FormatId;
    } catch {}
    return "format1";
  });
    ...
    fetch(`/api/profiles/${profileId}`)
      ...
      .then((body: { data: ResumeData }) => {
        ...
        const withContentCleared: ResumeData = {
          ...
          skills: [],
          experience: (loaded.experience ?? []).map((exp) => ({ ...exp, description: "" })),
        };
  ...
  const scrollCellIntoView = useCallback((row: number, col: number) => {
    requestAnimationFrame(() => {
      ...
      if (outer) {
        ...
        const visibleRight = outer.scrollLeft + outer.clientWidth;
  ...
  const handleScroll = useCallback(() => {
    ...
    const { scrollTop, clientHeight, scrollHeight, scrollLeft } = el;
  ...
  // Document-level Ctrl+C so copy always works (grid may not have focus on web)
  useEffect(() => {
    const handleDocKeyDown = (e: KeyboardEvent) => {
      const ctrl = e.ctrlKey || e.metaKey;
      if (!ctrl || e.shiftKey || (e.key !== "c" && e.key !== "C")) return;
  ...
  const handleDelete = async (id: string) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        console.error("Delete failed:", (data as { error?: string }).error);
        fetchApplications(profileFilterId);
  ...
  const handleDeleteMany = async (ids: string[]) => {
    ...
    const placeholders = ids.filter((id) =>
      applications.some((a) => a.id === id && isPlaceholder(a))
    );
    const realIds = ids.filter((id) => !placeholders.includes(id));
    setApplications((prev) => {
      pushHistory(prev);
    ...
    if (realIds.length) {
      try {
        await Promise.all(
          realIds.map((id) => fetch(`/api/job-applications/${id}`, { method: "DELETE" }))
        );
        void fetchDuplicateKeys();
      } catch (e) {
        console.error(e);
  ...
  return (
    <div className="flex flex-col min-h-0 flex-1 overflow-hidden container mx-auto px-4 py-4">
      <Card className="rounded-none border-0 shadow-none flex-1 flex flex-col min-h-0 overflow-hidden">
        <CardContent className="flex-1 flex flex-col min-h-0 overflow-hidden p-4">
          {loading ? (
            ...
            <>
              <div className="flex-shrink-0 flex flex-wrap items-center gap-6 pb-3 text-sm text-muted-foreground mx-auto">
              ...
              <div
                ref={outerScrollRef}
                className="flex flex-col flex-1 min-h-0 w-full min-w-0 overflow-x-auto overflow-y-hidden outline-none focus:outline-none"
              >
              <div
              ...
              >
                ...
                <div
                  ...
                  className="flex-1 min-h-0 min-w-0 overflow-y-auto overflow-x-hidden outline-none focus:outline-none"
              ...
              >
                ...
                >
                  ...
                  {dataRows.map((app, index) => {
                    ...
                    const renderInput = (key: ColumnKey, placeholder?: string) => (
                      <div className="absolute inset-0 flex items-stretch">
                        <input
                          autoFocus
                          className="flex-1 min-w-0 w-full h-full bg-background text-xs outline-none border-none px-2 py-0.5 box-border"
                    ...
                    return (
                      ...
                      >
                        <TableCell
                          ...
                          style={{ width: columnWidths.no, minWidth: columnWidths.no, maxWidth: 960 }}
                          onMouseDown={(e) => handleCellMouseDown(e, row, 0)}
                        >
                          {index + 1}
                        </TableCell>
                        <TableCell
                          ...
                          style={{ width: columnWidths.date, minWidth: columnWidths.date, maxWidth: 960 }}
                          onMouseDown={(e) => handleCellMouseDown(e, row, 1)}
                        ...
                        <TableCell
                          ...
                          style={{ width: columnWidths.company, minWidth: columnWidths.company, maxWidth: 960 }}
                          onMouseDown={(e) => handleCellMouseDown(e, row, 2)}
                          onDoubleClick={() =>
                        ...
                        <TableCell
                          ...
                          style={{ width: columnWidths.title, minWidth: columnWidths.title, maxWidth: 960 }}
                          onMouseDown={(e) => handleCellMouseDown(e, row, 3)}
                        ...
                        <TableCell
                          ...
                          style={{ width: columnWidths.jobUrl, minWidth: columnWidths.jobUrl, maxWidth: 960 }}
                          onMouseDown={(e) => handleCellMouseDown(e, row, 4)}
                          onDoubleClick={() =>
              ...
              >
                <ExternalLink className="h-3.5 w-3.5" />
                        ...
                        <TableCell
                          ...
                          onMouseDown={(e) => handleCellMouseDown(e, row, 5)}
                        >
                          {(() => {
                                ...
                                <div className="inline-flex items-center gap-0.5">
                                  <button
                                    ...
                                    onClick={(ev) => {
                                      ev.stopPropagation();
                                      window.open(
                                        `/api/job-applications/${app.id}/pdf`,
                                        "_blank",
                                        "noopener,noreferrer"
                                      );
                                    }}
                  ...
                  {showEmptyRows && Array.from({ length: emptyRowCount }, (_, i) => {
                    ...
                    const emptyIndex = dataRows.length + i;
                    return (
                    ...
                    return (
                      <TableRow className="border-b border-border bg-muted/30">
                        <TableCell
                          ...
                          onMouseDown={(e) => handleCellMouseDown(e, addMoreRow, 0)}
                        >
                          <Button
                          type="button"
                          variant="outline"
      ...
      <Dialog open={addOpen} onOpenChange={(open) => { setAddOpen(open); if (!open) setApplyApplication(null); }}>
        <DialogContent className="w-[90vw] max-w-6xl h-[90vh] min-h-[80vh] overflow-hidden flex flex-col p-0 gap-0" aria-describedby={undefined} showClose={false}>
          ...
          <DialogHeader className="flex-shrink-0 px-6 pt-4 pb-3 flex-row items-center justify-between gap-3">
            ...
            <form
              ...
              className="flex flex-1 items-center justify-center gap-2 min-w-0 max-w-2xl mx-auto"
            >
              <Input
                id="add-company"
                ...
                return (
                  ...
                  >
                    <span className="flex items-center gap-0.5">
                      <FileText className="h-3.5 w-3 shrink-0" strokeWidth={2} />
                      <span>{num}</span>
                    </span>
                  </button>
                );
                      ...
                      onClick={async () => {
                        ...
                        try {
                          ...
                          try {
                            ...
                            setAiButtonLoading((s) => ({ ...s, 1: false }));
                          }

                          // Step 2: last company bullets
                          setAiButtonLoading((s) => ({ ...s, 2: true }));
                          let lastBullets = "";
                          try {
                            if (lastCompanyName) {
                            ...
                            setAiButtonLoading((s) => ({ ...s, 2: false }));
                          }

                          // Step 3: summary
                          setAiButtonLoading((s) => ({ ...s, 3: true }));
                          try {
                            ...
                            setAiButtonLoading((s) => ({ ...s, 3: false }));
                          }

                          // Step 4: skills
                          setAiButtonLoading((s) => ({ ...s, 4: true }));
                          try {
                            ...
                            setAiButtonLoading((s) => ({ ...s, 4: false }));
                          }
                        } finally {
              ...
              <div ref={applyModalContentRef} className="flex-1 min-h-0 border rounded bg-muted/40 p-2 overflow-y-auto flex flex-col gap-2">
                {modalDataLoading ? (
                  ...
                  <ApplicationResumeEditor
                    ...
                    onExperienceDescriptionChange={(expId, description) => {
                      setModalContentVersion((v) => v + 1);
                      setModalResumeData((prev) => {
                        const current = prev ?? defaultResumeData;
                        return {
                    ...
                    onExperienceRoleChange={(expId, role) => {
                      setModalContentVersion((v) => v + 1);
                      setModalResumeData((prev) => {
                        const current = prev ?? defaultResumeData;
                        return {
      ...
      <Dialog open={!!viewJobDescriptionId} onOpenChange={(open) => !open && setViewJobDescriptionId(null)}>
