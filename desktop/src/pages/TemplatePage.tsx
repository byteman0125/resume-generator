...
export function TemplatePage() {
  const { formatId: formatIdParam } = useParams<"formatId">();
  const navigate = useNavigate();
  const formatId = formatIdParam ?? "";
  ...
  useEffect(() => {
    if (!validFormat) {
      navigate("/template/format1", { replace: true });
      return;
    }
    loadStyle();
  }, [validFormat, formatId, navigate, loadStyle]);
